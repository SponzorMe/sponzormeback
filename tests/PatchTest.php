	...
	public function testPatchUsers()
  {
       ...
       for ($i=0; $i < $this->maxInsertions ; $i++) {
         ...
         $user=[
    	  ...
    	  $response = $this->call('PATCH', '/users/'.$faker->randomElement($users->toArray()), $user, array(), array(),array("HTTP_AUTHORIZATION"=>"Basic $this->loginToken"));
	...
	public function testPatchCategories()
  {
       ...
       for ($i=0; $i < $this->maxInsertions ; $i++) {
         ...
         $category=[
    	  ...
    	  $response = $this->call('PATCH', '/categories/'.$faker->randomElement($categories->toArray()), $category, array(), array(),array("HTTP_AUTHORIZATION"=>"Basic $this->loginToken"));
    		$this->assertEquals(200, $response->getStatusCode());
        $this->seeInDatabase('categories', ['title' => $category["title"]]);
      }
	}
	public function testPatchInterests()
  {
       $faker = Faker\Factory::create();
			 $categories = Category::all()->lists('id');
			 //var_dump($categories);
       for ($i=0; $i < $this->maxInsertions ; $i++) {
         ...
         $interest=[
    	  ...
    	  $response = $this->call('PATCH', '/interests_category/'.$faker->randomElement($interests->toArray()), $interest, array(), array(),array("HTTP_AUTHORIZATION"=>"Basic $this->loginToken"));
    		$this->assertEquals(200, $response->getStatusCode());
        $this->seeInDatabase('interests_categories', ['name' => $interest["name"]]);
      }
	}
	public function testPatchEventType()
  {
       $faker = Faker\Factory::create();
       for ($i=0; $i < $this->maxInsertions ; $i++) {
         ...
         $eventType=[
    	  ...
    	  $response = $this->call('PATCH', '/event_types/'.$faker->randomElement($eventTypes->toArray()), $eventType, array(), array(),array("HTTP_AUTHORIZATION"=>"Basic $this->loginToken"));
    		$this->assertEquals(200, $response->getStatusCode());
	...
	public function testPatchEvents()
  {
       ...
       for ($i=0; $i < $this->maxInsertions ; $i++) {
         ...
         $event=[
    	  ...
    	  $response = $this->call('PATCH', '/events/'.$faker->randomElement($events->toArray()), $event, array(), array(),array("HTTP_AUTHORIZATION"=>"Basic $this->loginToken"));
    		$this->assertEquals(200, $response->getStatusCode());
	...
	public function testPatchPerks()
  {
       ...
       for ($i=0; $i < $this->maxInsertions ; $i++) {
         ...
         $perk=[
    	  ...
    	  $response = $this->call('PATCH', '/perks/'.$faker->randomElement($perks->toArray()), $perk, array(), array(),array("HTTP_AUTHORIZATION"=>"Basic $this->loginToken"));
    		$this->assertEquals(200, $response->getStatusCode());
	...
	public function testPatchPerkTasks()
  {
       ...
       for ($i=0; $i < $this->maxInsertions ; $i++) {
         ...
         $perkTaks=[
    	  ...
    	  $response = $this->call('PATCH', '/perk_tasks/'.$faker->randomElement($perkTasks->toArray()), $perkTaks, array(), array(),array("HTTP_AUTHORIZATION"=>"Basic $this->loginToken"));
    		$this->assertEquals(200, $response->getStatusCode());
	...
	public function testPatchTaskSponzor()
  {
       ...
       for ($i=0; $i < $this->maxInsertions ; $i++) {
	         ...
	         $taskSponzor=[
	    	  ...
	    	  $response = $this->call('PATCH', '/task_sponzor/'.$faker->randomElement($taskSponzors->toArray()), $taskSponzor, array(), array(),array("HTTP_AUTHORIZATION"=>"Basic $this->loginToken"));
	    		$this->assertEquals(200, $response->getStatusCode());
	...
	public function testPatchUserCategory()
  {
       ...
       for ($i=0; $i < $this->maxInsertions ; $i++) {
         ...
         $userCategory=[
    	  ...
    	  $response = $this->call('PATCH', '/user_categories/'.$faker->randomElement($usersCategory->toArray()), $userCategory, array(), array(),array("HTTP_AUTHORIZATION"=>"Basic $this->loginToken"));
    		$this->assertEquals(200, $response->getStatusCode());
	...
	public function testPatchUserInterests()
  {
       ...
       for ($i=0; $i < $this->maxInsertions ; $i++) {
         ...
         $userInterests=[
    	  ...
    	  $response = $this->call('PATCH', '/user_interests/'.$faker->randomElement($usersInterest->toArray()), $userInterests, array(), array(),array("HTTP_AUTHORIZATION"=>"Basic $this->loginToken"));
    		$this->assertEquals(200, $response->getStatusCode());
